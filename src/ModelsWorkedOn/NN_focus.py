...
#With jacks preprocessing and outlier removal .15255 for NN
...
def main():
    ...
    pipeline = make_pipeline(
        MinMaxScaler(),
        MLPRegressor(
            hidden_layer_sizes=(128, 64, 50), # (128, 64, 50) .15255
            activation='relu',
            solver='adam',
            alpha=0.0001,
            learning_rate_init=0.001,
            max_iter=1000, #1000 org .15255
            random_state=42
        )
    )
    ...
    submission.to_csv(SUBMISSION_NN_FILE_PATH, index=False)
    get_logrmse(SUBMISSION_NN_FILE_PATH, SAMPLE_SUBMISSION_FILE_PATH)
