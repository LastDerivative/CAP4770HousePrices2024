#Track down the info to get a better understanding of progress where eveything is at.
#No tuning and no jack: 0.43053
#No tuning with jack:
#Tuning .18065 and no jack
#With jacks preprocessing and outlier removal .15049 for NN kagle (DONE)


#After tuning, need graph showing untunned data
...
PREDICTIONS_NN_FILE_PATH = 'predictions_NN.csv'
...
def main():
    ...
    predictions_nn_file_path = PREDICTIONS_NN_FILE_PATH
    ...
    pipeline = train_neural_network(X, y)
    ...
    submission.to_csv(predictions_nn_file_path, index=False)
    get_logrmse(predictions_nn_file_path, sample_submission_file_path)
